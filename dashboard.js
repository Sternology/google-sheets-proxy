...
const mapRow = (headers, row) => {
    ...
    headers.forEach((h, i) => {
        if (!h || !row[i]) return;
        const header = h.trim().toLowerCase(); // Normalize header to lowercase
        const val = row[i];
        ...
        // Metrics
            ...
            if (!isNaN(num)) {
                // FIXED: Comparisons are now strictly lowercase
...
const BudgetAction = ({ client }) => {
    ...
    else if (Math.abs(change) < 2) { status = 'optimal'; msg = 'Spend is Optimal'; }
            ...
            {status !== 'optimal' && status !== 'critical' && (
                <div className="grid grid-cols-2 gap-6 text-sm">
                    <div>
                        ...
                        <div className="flex justify-between"><span>Current</span><strong>{formatMoney(client.currentDailyBudget)}</strong></div>
                    ...
                    <div>
                        ...
                        {recGoogle > 0 && (
                            <div className="flex justify-between items-center mb-1">
                                <span className="text-xs bg-orange-100 text-orange-800 px-1.5 rounded">Google</span>
                                <span className="font-mono">{formatMoney(recGoogle)}</span>
                            </div>
                        )}
                        {recFB > 0 && (
                            <div className="flex justify-between items-center">
                                <span className="text-xs bg-blue-100 text-blue-800 px-1.5 rounded">Meta</span>
                                <span className="font-mono">{formatMoney(recFB)}</span>
                            </div>
                        )}
                    </div>
                </div>
...
const ClientDashboardCard = ({ client }) => {
    ...
    const paceColor = client.status === 'HOT' ? '#EF4444' : client.status === 'COLD' ? '#3B82F6' : '#10B981';
    ...
    const load = async () => {
        ...
        try {
            ...
            const results = await Promise.all(clients.map(async client => {
                ...
                let stats = { total: 0, ctrSum: 0, ctrCount: 0, careApps: 0, nurseApps: 0, supportApps: 0, gDaily: 0, fbDaily: 0 };
                ...
                const fetchTab = async (suffix) => {
                    try {
                        ...
                        if (suffix === 'FB') fbBudgets.forEach(v => stats.fbDaily += v);
                ...
                return {
                    ...
                    googleDailyBudget: stats.gDaily, facebookDailyBudget: stats.fbDaily,
